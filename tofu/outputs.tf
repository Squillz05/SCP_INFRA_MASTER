...
output "workstation_floating_ips" {
  ...
  value = merge(
    { for k, v in openstack_networking_floatingip_v2.blue_windows : openstack_compute_instance_v2.blue_windows[k].name => v.address },
    { for k, v in openstack_networking_floatingip_v2.blue_linux : openstack_compute_instance_v2.blue_linux[k].name => v.address },
    { for k, v in openstack_networking_floatingip_v2.red_kali : openstack_compute_instance_v2.red_kali[k].name => v.address },
    { for k, v in openstack_networking_floatingip_v2.red_windows : openstack_compute_instance_v2.red_windows[k].name => v.address },
  )
}
...
output "workstation_main_nat_interface_ips" {
  ...
  value = merge(
    { for k, v in data.openstack_networking_port_v2.blue_windows_main_nat : openstack_compute_instance_v2.blue_windows[k].name => one(v.all_fixed_ips) },
    { for k, v in data.openstack_networking_port_v2.blue_linux_main_nat : openstack_compute_instance_v2.blue_linux[k].name => one(v.all_fixed_ips) },
    { for k, v in data.openstack_networking_port_v2.red_kali_main_nat : openstack_compute_instance_v2.red_kali[k].name => one(v.all_fixed_ips) },
    { for k, v in data.openstack_networking_port_v2.red_windows_main_nat : openstack_compute_instance_v2.red_windows[k].name => one(v.all_fixed_ips) },
  )
}
